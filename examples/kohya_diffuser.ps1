...
$v2 = "" # set to "--v2" to force the use of v2 model
$v_model = "" # set to "--v_parameterization" to force the use of v2 v model like the 768v one
...
$output_dir = "D:\models\test\samdoesart2\model\"
...
$convert_to_safetensor = 1 # set to 1 to convert resulting diffuser to ckpt
...
if (Test-Path "$output_dir\last" -PathType Container) {
    ...
    if ($convert_to_safetensor) {
        Write-Host("Converting diffuser model $output_dir\last to $output_dir\last.safetensor")
        python "$kohya_diffusers_fine_tuning_repo_path\tools\convert_diffusers20_original_sd.py" `
            $output_dir\last `
            $output_dir\last.safetensor `
            --$save_precision
    }
}

# define a list of substrings to search for inference file
$substrings_sd_model = ".ckpt", ".safetensor"
